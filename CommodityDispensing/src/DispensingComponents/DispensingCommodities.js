...
function mergeData(data) {
    ...
    data.dataSets.dataSetElements.map((d) => {
        ...
        d.dataElement.dataElementGroups.map((c) => {
            if (c.id !== "Svac1cNQhRS") {
                const category = mergeData.filter((e) => e.name === c.name);
...
/**
 * Creates a DispensingCommodities component which functions as a form container to dispense commodites.
 * props/args: props.me (object) - Data from the merged data from the maindataquery,
 *       passed through DispenseTabs
 * Returns: a finshed DispenseCommodities component
*/
export function DispensingCommodities(props) {
    ...
    const [dataStoreMutate, { mutateDSLoading, mutateDSError }] =
        useDataMutation(fetchDataStoreMutationQuery());
    ...
    const formHandler = async (values) => {
        ...
        values.dispensed.forEach((value) => {
            const consumption = dataValues.find(function (obj) {
                return (
            ...
            lifeCommodityMutation.push({
                ...
                period: props.me.currentPeriod,
                orgUnit: props.me.orgUnit,
                value: parseInt(consumption) + parseInt(value.value),
            });
        ...
        if (success) {
            ...
            //save the dispensed commodity(s) to Datastore on success
            ...
            let combindedDataStoreData = mergeDataStoreData(
                refetchingData.dispensingDataStoreData,
                newHistoryElement,
            ...
            //On success: Reset form and give user feedback
                                        ...
                                        handleSubmit()
                                            .then(function (response) {
                                                setAlertInformation({
                                                    ...
                                                    permanent: false,
                                                    message:
                                                        "Successfuly dispensed the commodity/ies",
                                                });
                                                refetch();
                                                form.restart();
                                            })
